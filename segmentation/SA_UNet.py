...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training == True:
            x = dropBlock(x, 
                          block_size=(x.shape[-1]//self.block_size if x.shape[-1]//self.block_size%2==1 else x.shape[-1]//self.block_size+1), 
...
class SA_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training == True:
            b = dropBlock(b, 
                          block_size=(b.shape[-1]//self.block_size if b.shape[-1]//self.block_size%2==1 else b.shape[-1]//self.block_size+1), 
                          keep_prob=self.keep_prob)
        b = self.bn1(b)
        b = self.relu(b)
